...
namespace jeanf.universalplayer
{
    ...
    public class XrModeHud : MonoBehaviour
    {
        ...
        {
            if (_broadcaster == null) _broadcaster = FindAnyObjectByType<BroadcastControlsStatus>();
            if (_movement == null) _movement = FindAnyObjectByType<PlayerMovement>();
            if (_camera == null) _camera = FindAnyObjectByType<FPSCameraMovement>();
            if (_noPeeking == null) _noPeeking = FindAnyObjectByType<NoPeeking>();
        }
        ...
        private void AppendCameraState(StringBuilder sb)
        {
            ...
            if (cam != null)
            {
                ...
                sb.Append("camera: '").Append(cam.name).Append("' enabled=").Append(cam.enabled)
                  ...
                  .Append(" rect=").Append(cam.rect)
                  .Append(" pixelRect=").Append(cam.pixelRect)
                  .Append(" aspect=").Append(cam.aspect.ToString("F3"))
        ...
        private static void AppendXrState(StringBuilder sb)
        {
            ...
            sb.Append("XR: settings.enabled=").Append(XRSettings.enabled)
              ...
              .Append(" mirrorBlitMode=").Append(XrDisplayLifecycle.PreferredMirrorBlitMode)
              .Append(" gameViewRenderMode=").Append(XRSettings.gameViewRenderMode)
              .Append(" eyeTexture=").Append(XRSettings.eyeTextureWidth).Append('x').Append(XRSettings.eyeTextureHeight)
